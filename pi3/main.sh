...
then 
  ...
  mkdir -p              ${rootmnt}/install/pi3
  cp -a ${wd}/*         ${rootmnt}/install/pi3
fi
...
in 
    ...
    ("extract") \
        ...
        ;;
    ("base") \
        arch-chroot "${rootmnt}" /install/pi3/4-base.sh  # Absolute path, in chroot dir !!!
        next="yay"
        ;;
    ("yay") \
        # !! skipping this one for now
        #arch-chroot "${rootmnt}" /install/pi3/5-yay.sh   # Absolute path, in chroot dir !!!
        printf "Stage yay in progress: run it manually: here is a qemu-chroot shell \n"
        arch-chroot ${rootmnt} /bin/bash
        next="finish"
        ;;
    (*) \
