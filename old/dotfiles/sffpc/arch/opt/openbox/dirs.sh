...
Configuration:
/usr/bin/bl-places-pipemenu may be edited directly for system-wide changes,
...
output="$output
<item label=\"pcmanfm here...\">
    <action name=\"Execute\">
        <command>
        pcmanfm -n &apos;$pathe_apos&apos;
        </command>
    </action>
</item>
<item label=\"terminal here...\">
    <action name=\"Execute\">
        <command>
        terminator --command=\"cd ${pathe_apos}; bash;\"
        </command>
    </action>
</item>
<separator />
